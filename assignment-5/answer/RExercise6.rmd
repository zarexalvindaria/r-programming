...
ANS: 
As seen on the graph, the higher the population, the higher the difference between the per capita value is and the lower the population, the more uniform the per capita is.
...
ggplot(data = binomial.data, mapping = aes(x = population, y = per.capita)) + geom_point() + 
  ...
  labs(title = "Residual\n", color = "Legend Title\n") + 
  scale_color_manual(labels = c(">3 residuals")) +
  theme_bw()
```
...
# Computing the number of residual with value greater than 3
residual.count <- sum(binomial.data['residual'] > 3)
```

ANS: 
How many counties had a standardized residual exceeding 3?
There are `r residual.count` counties with greater than 3 residual.
